...
unpack( <<?PPSPP_LIVE_SIGNATURE_ALGORITHM,
    ...
    Algorithm = case Maybe_Algorithm of
                    %% TODO we can parse this yet but need normative
                    %% algorithm names
...
unpack( <<?PPSPP_LIVE_DISCARD_WINDOW, Maybe_Discard_Window/binary>>,
    ...
    %%Length = case orddict:fetch(chunk_addressing_method, Options) of
    ...
    Window = case Window0 of
                 16#ffffffff         ->
                     ppspp_live_discard_window_keep_all_chunks;
                 16#ffffffffffffffff ->
                     ppspp_live_discard_window_keep_all_chunks;
                 _ -> Window0
             end,
...
unpack( <<?PPSPP_SUPPORTED_MESSAGES,
    ...
    Supported_Message_Types =
        case Maybe_Supported_Message_Types of
            %% TODO probably needs a custom bitmatching
            %% parser
            _ ->
                ppspp_supported_messages_not_yet_implemented
        end,
    %% Options = orddict:store(ppspp_supported_message_types,
    %%                         Supported_Message_Types, Options0),
    unpack(Maybe_Options, Options#{ppspp_supported_message_types =>
                                       Supported_Message_Types});
...
get_chunk_addressing_method(#{chunk_addressing_method := Method} = _Options) ->
    Method.

%% @doc Returns CIPM for the provided swarm
-spec get_content_integrity_check_method(options()) -> any().

get_content_integrity_check_method(#{content_integrity_check_method := Method}
                                   = _Options) ->
    Method.
...
get_merkle_hash_tree_function(#{merkle_hash_tree_function := HashFunc}
                              = _Options) ->
    HashFunc.
...
get_minimum_version(#{minimum_version := MinVersion} = _Options) ->
    MinVersion.
...
get_swarm_id(#{swarm_id := SwarmId} = _Options) ->
    SwarmId.

%% @doc Returns the swarm chunk size
-spec get_chunk_size(options()) -> pos_integer().
get_chunk_size(#{chunk_size := Size} = _Options) ->
    Size.
...
get_maximum_supported_version(#{supported_version := Version} = _Options) ->
    Version.

%% @doc helper unwrapper to pull out components from a datagram orddict
%% @end
-spec get_options(map()) -> options().
get_options(#{options := Options} = _MessageBody) ->
    Options.
...
use_minimum_options() ->
    #{options => #{minimum_version => ?PPSPP_RFC_VERSION}}.
    %% {options, orddict:from_list([{minimum_version, ?PPSPP_RFC_VERSION}])}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% @doc Pack an map of erlang terms into a binary PPSPP message segment
